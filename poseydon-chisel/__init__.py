...
def Fasta2Seqs(fileloc, idents = False): 
    seqs = []
    ids = []
...
def CurrentMerger(SigCurrent, mode = np.mean): 
    return [mode(z, axis = 1).reshape(-1,1) for z in SigCurrent] 

def CurrentModifier(SigCurrent, modifier, mode = np.multiply): 
    #modifier is a list, should match len of SigCurrent. Could be a single value ex. [-1] 
    return [mode(z, m) for z,m in zip(SigCurrent, modifier)]
...
def CurrentInterpolator(SigCurrent, reso, window = None, threshold = 0, threshsign = np.greater): 
    ...
    for t in range(SigCurrent[0].shape[1]): 
        ...
        if dtype is not None: nt = [g.astype(dtype) for g in nt]
        AT.append(nT) 
...
def CurrentExpander(SigCurrent, reso, BS_size, BS_ids, select_BS_ids = None, filler = 0): 
    ...
    for iz, z in enumerate(select_BS_ids):
        ...
        nT.append(np.append(repd, np.repeat(filler,lg-len(repd))).reshape(-1,1))
...
def CurrentWindower(SigCurrent, reso = 1, mode = np.mean, window = None, center = None, extend = None, dtype = None): 
    ...
    for z in SigCurrent: 
        ...
        if center is not None: t = np.concatenate([np.repeat(center, window // 2), t])
        if extend is not None: t = np.concatenate([t, np.repeat(extend, lz - len(t))])
    ...
    for z in SigCurrent: 
        ...
        if double == True: 
            ...
            for ix in idx: 
                ...
                g1, g2 = (np.zeros((window2, f)) for f in [j, 1])
...
def Seqs2OHE(seqs, expand_dim = None, customdict = None):
    ...
    for seq in seqs:
        seq = seq.upper()
        OHEseq = [OHEdict[nuc] for nuc in seq]
        OHEtotal.append(OHEseq)
    OHEtotal = np.squeeze(np.array(OHEtotal, dtype='int8'))
...
def Seq2PWMScore(seq, PWM, pieces = None, RC = True, center = None, extend = None, dtype = None): 
    ...
    if center is not None: scores = np.concatenate([np.repeat(center, lp // 2), scores])
    if center is not None: scores = np.concatenate([scores, np.repeat(extend, ls - len(scores))])
    ...
    for t in select_BS_ids: 
        if t in BS_ids: 
            ...
            if parrellel is not None: 
                ...
                if center is not None: scores = np.vstack([np.repeat(center, lp // 2).reshape(-1,1), scores])
                if center is not None: scores = np.vstack([scores, np.repeat(extend, ls - len(scores)).reshape(-1,1)])
...
def SigCurrent2Slice(SigCurrent, Marker, BS_ids, select_BS_ids = None, Vsize = 1000, reso = 5, newreso = None, resomode = np.mean, exact = True, Vpad = 2, stranded = False): 
...
def SigCurrent2Pack(SigCurrent, Markers, BS_ids, select_BS_ids = None, Vsize = 1000, reso = 5, newreso = None, resomode = np.mean, areas = None, exact = False, stranded = False): 
    ...
    Pack = []
    for ir in Markers.index:
        Pack.append(SigCurrent2Slice(SigCurrent, Markers.iloc[ir] , BS_ids, select_BS_ids, Vsize = Vsize, reso = reso, newreso = None, resomode = np.mean, exact = exact, Vpad = 2, stranded = stranded))
...
def LowerResProfilePack(ProfilePack, reso, newreso, resomode = np.mean): 
...
def Traits2Pack(Markers, cols = None, vectorize = None): 
...
''' def KmerFinder(SeqCurrent, kmer, center = None, extend = None): 
    ...
    for z in SeqCurrent: 
        lz = len(z)
        m = np.repeat(0, lz-lkmer + 1)
        for ir, r in enumerate(z[:-lkmer]): 
            j = z[ir:ir+lkmer]
            if j == kmer or reverse_complement(j) == kmer: 
                m[z][ir] = 1
        if center is not None: m = np.concatenate([np.repeat(center, window // 2), m])
        if extend is not None: m = np.concatenate([t, np.repeat(extend, lz - len(m))])
        ms.append(m.reshape(-1,1)) 

    return ms ''' 

'''def Seq2PWMScore(seq, PWM, pieces = None, center = None, extend = None): 
    # introduce pieces for long seq 
    ...
    for i in range(pieces): 
        ...
        scores.append(np.max([s1, s2[::-1]], axis = 0))
...
''' def SigCurrent2Markers(SigCurrent, BS_ids, select_BS_ids = None, reso = 1, top = None, smallest = False): 
    #makes a sigCurrent out of non zero values. Recommended to make a BinCurrent from CurrentThresholder first or filter out values you dont want
    
    if top is not None: 
        allvals = np.concatenate(SigCurrent, axis = 0).reshape(-1)
        k = 1 if smallest == True else -1
        thresh = allvals[np.argsort(allvals)[::k]][top]
        pdx = [np.where(z > thresh)[0] for z in SigCurrent] if smallest == False else [np.where(z < thresh)[0] for z in SigCurrent]
    ...
    for ip, p in enumerate(pdx): 
        ...
        vals = (SigCurrent[ip][p]).reshape(-1)
        subseqs, strand, size = [np.repeat(x, lp) for x in [select_BS_ids[ip], 0, 1]]
        Markers.append(np.stack([subseqs, p, strand, size, vals], axis = 1))
    Markers = np.vstack(Markers)
    d = pd.DataFrame(Markers, columns = ['subseq', 'loc', 'strand', 'size', 'val'])
    d[['loc', 'strand', 'size']] = d[['loc', 'strand', 'size']].astype(int)
    d[['val']] = d[['val']].astype(float)
    
    d = d.sort_values(by=['subseq', 'loc', 'strand'])
